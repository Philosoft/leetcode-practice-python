...
class Solution(TestCase):
    def test_it(self):
        options = [
            ("example 1", "aaabcbc", "3[a]2[bc]"),
            ("example 2", "accaccacc", "3[a2[c]]"),
            ("example 3", "abcabccdcdcdef", "2[abc]3[cd]ef"),
            ("100 of leetcode", "leetcode" * 100, "100[leetcode]"),
        ]

        for name, expected, s in options:
            with self.subTest(name):
                self.assertEqual(expected, self.decodeString(s))
    ...
    def decodeString(self, s: str) -> str:
        ...
        for char in list(s):
            if char != "]":
                ...
                buf = []
                while result[-1] != "[":
                    buf.append(result.pop())
                result.pop()
                times = []
                while result and result[-1].isdigit():
                    times.append(result.pop())
                times = int("".join(reversed(times)))
                result.append("".join(reversed(buf)) * times)
