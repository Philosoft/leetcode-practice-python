...
class Solution(TestCase):
    def test_example_1(self):
        ...
        self.assertEqual([-1, 3, -1], self.nextGreaterElement(nums1, nums2))

    def test_example_2(self):
        nums1 = [2, 4]
        nums2 = [1, 2, 3, 4]

        self.assertEqual([3, -1], self.nextGreaterElement(nums1, nums2))

    def nextGreaterElement(self, nums1: List[int], nums2: List[int]) -> List[int]:
        next_greater: Dict[int, int] = defaultdict(lambda: -1)
        search_for: Set[int] = set()
        for idx, n in enumerate(nums2):
            to_remove: Set[int] = set()
            for query in search_for:
                if n > query:
                    to_remove.add(query)
                    next_greater[query] = n
            search_for ^= to_remove
            search_for.add(n)
