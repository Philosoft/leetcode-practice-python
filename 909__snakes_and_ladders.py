...
class Solution(TestCase):
    def test_example_1(self):
        ...
        self.assertEqual(4, self.snakesAndLadders(board))

    def test_example_2(self):
        board = [[-1, -1], [-1, 3]]

        self.assertEqual(1, self.snakesAndLadders(board))

    def test_from_discussion(self):
        board = [[-1, 1, 2, -1], [2, 13, 15, -1], [-1, 10, -1, -1], [-1, 6, 2, 8]]

        self.assertEqual(2, self.snakesAndLadders(board))

    def test_failed_leetcode(self):
        board = [
            [-1, -1, -1],
            [-1, 9, 8],
            [-1, 8, 9],
    ...
    def test_failed_leetcode_211(self):
        board = [[1, 1, -1], [1, 1, 1], [-1, 1, 1]]

        self.assertEqual(-1, self.snakesAndLadders(board))

    def snakesAndLadders(self, board: List[List[int]]) -> int:
        # padded flat array for easy addressing
        flat_board = [666]
        inverted = False
        for row in reversed(board):
            for cell in reversed(row) if inverted else row:
        ...
        while q:
            for _ in range(len(q)):
                ...
                for possible_step in range(current_position + 1, current_position + 7):
                    if possible_step not in visited:
                        if possible_step == target:
                            return steps + 1
                        visited.add(possible_step)
                        if flat_board[possible_step] != -1:
                            if flat_board[possible_step] == target:
                                return steps + 1
                            q.append(flat_board[possible_step])
                        else:
                            q.append(possible_step)
